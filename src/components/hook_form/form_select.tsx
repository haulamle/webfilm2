...
export const FormSelect = (props: IProps) => {
  ...
  const renderOptions = (value: string) => {
    ...
    if (options.length === 0) {
      return <MenuItem disabled>Không có lựa chọn nào</MenuItem>
    }
  ...
  return (
    <Controller
      ...
      render={({ field: { value, onChange }, fieldState: { error, invalid } }) => (
        <FormControl sx={sx} fullWidth margin={margin} size={size} error={invalid} required={required}>
          ...
          {invalid && <FormHelperText>{error?.message || helperText}</FormHelperText>}
        </FormControl>
      )}
    />
  )
